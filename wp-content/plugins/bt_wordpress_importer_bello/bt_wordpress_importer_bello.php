<?php
/*
Plugin Name: BoldThemes WordPress Importer (Bello)
...
function bt_import_ajax_callback() {
	...
	$reader_index = $_POST['reader_index'];
		 ...
		 */
		 ...
		 if ( ! function_exists( 'bt_export' ) ) {
			function bt_export() {
				...
				// listing-category custom fields & settings
				...
				$cf_arr = array();
				$cf_arr_settings = array();

				foreach( $terms as $t ) {
					$term_id = $t->term_id;
					$term_slug = $t->slug;
					$cf = get_option( "taxonomy_term_$term_id" );
					$listing_fields = $cf['listing_fields'];
					$listing_fields = trim( $listing_fields );
					$cf_arr[ $term_slug ] = $listing_fields;

					$cf_settings = $cf['cf_settings'];
					$cf_arr_settings[ $term_slug ] = $cf_settings;
				}

				$export[ 'listing_cat_custom_fields' ] = $cf_arr;
				$export[ 'listing_cat_custom_field_settings' ] = $cf_arr_settings;
		...
		if ( ! function_exists( 'bt_import_end' ) ) {
			function bt_import_end() {
				...
				if ( $match ) {
					...
					if ( isset( $data_arr['listing_cat_custom_fields'] ) ) {
						foreach ( $data_arr['listing_cat_custom_fields'] as $slug => $cf ) {
							$t = get_term_by( 'slug', $slug, 'listing-category' );
							$t_id = $t->term_id;
							$term_meta = get_option( "taxonomy_term_$t_id" );
							$term_meta['listing_fields'] = $cf;
							update_option( "taxonomy_term_$t_id", $term_meta );
						}
					}

					if ( isset( $data_arr['listing_cat_custom_field_settings'] ) ) {
						foreach ( $data_arr['listing_cat_custom_field_settings'] as $slug => $cf_settings ) {
							$t = get_term_by( 'slug', $slug, 'listing-category' );
							$t_id = $t->term_id;
							$term_meta = get_option( "taxonomy_term_$t_id" );
							$term_meta['cf_settings'] = $cf_settings;
							update_option( "taxonomy_term_$t_id", $term_meta );
						}
					}
