...
class Query implements AssociationQuery {
	...
	public function element_status( $statuses, RelationshipRole $for_role = null ) {
		...
		return $this->condition_factory->element_status( $statuses, $for_role );
	}
	 ...
	 */
	private function get_unnecessary_wpml_table_joins( AndOperator $root_condition ) {
		if ( ! $this->wpml_service->is_wpml_active_and_configured() ) {
		...
		foreach ( $this->result_transformation->get_maximum_requested_roles() as $role ) {
			/** @var HasDomainAndType|null $condition_for_role */
