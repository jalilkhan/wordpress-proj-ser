...
class TranslatableElementSelector extends AbstractSelector {
	...
	private function configure_translatable_role( RelationshipRole $role ) {
		...
		$display_as_translated_value = 1;
		$standard_translate_value = 2;
		$non_translatable_value = 3;
		$autodraft_mode_value = 4;
		...
		// "translation_mode" JOIN is for. It selects any existing post from the translation group and determines
		// the translation mode based on its post type (we rely pretty strongly on the fact that all posts within
		// one translation group have the same type).
		//
	...
	private function get_in_display_as_translated_element_types( $post_types_by_translatability ) {
		if ( $this->force_display_as_translated_mode ) {
			return $this->filter_and_format_post_types_by_translatability(
				...
				static function ( $value ) { return WpmlService::MODE_DONT_TRANSLATE !== $value; }
			);
		}

		return $this->filter_and_format_post_types_by_translatability(
			$post_types_by_translatability,
			static function ( $value ) { return WpmlService::MODE_DISPLAY_AS_TRANSLATED === $value; }
		);
	}
	...
	private function get_in_standard_translate_element_types( $post_types_by_translatability ) {
		...
		return $this->filter_and_format_post_types_by_translatability(
			$post_types_by_translatability,
			static function ( $value ) { return WpmlService::MODE_TRANSLATE === $value; }
		);
	}
