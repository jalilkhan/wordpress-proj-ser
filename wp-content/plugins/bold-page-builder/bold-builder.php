...
function bt_bb_enqueue() {
	...
	wp_enqueue_script( 'bt_bb', plugins_url( 'script.min.js', __FILE__ ), array( 'jquery' ) );
	wp_enqueue_script( 'bt_bb_jsx', plugins_url( 'build/jsx.min.js', __FILE__ ), array( 'jquery' ) );
	wp_enqueue_script( 'bt_bb_autosize', plugins_url( 'autosize.min.js', __FILE__ ) );
	wp_enqueue_script( 'bt_bb_imagesloaded', plugins_url( 'imagesloaded.pkgd.min.js', __FILE__ ), array( 'jquery' ) );
...
function bt_bb_enqueue_fe_always() {
	wp_enqueue_script( 'bt_bb_fe', plugins_url( 'script_fe.js', __FILE__ ), array( 'jquery' ) );
}
...
function bt_bb_wp_head() {
	...
	if ( isset( $opt_arr[ $post_id ] ) ) {
		echo '<style>' . wp_strip_all_tags( $opt_arr[ $post_id ] ) . '</style>';
	}
...
function bt_bb_enqueue_content_elements() {
	...
	wp_enqueue_script( 'bt_bb', plugins_url( 'content_elements_misc/js/content_elements.js', __FILE__ ), array( 'jquery' ) );
...
function bt_bb_get_custom_css() {
	...
	$opt_arr = get_option( 'bt_bb_custom_css' );
	if ( isset( $opt_arr[ $post_id ] ) ) {
		echo wp_strip_all_tags( $opt_arr[ $post_id ] );
	}
	die();
}
...
function bt_bb_js_settings() {
	...
	?>
		jQuery( '#bt_bb_dialog' ).on( 'click', '.bt_bb_button_save_custom_css', function( e ) {
			...
			window.bt_bb_custom_css = b64EncodeUnicode( css );
...
if ( ! class_exists( 'BoldThemes_BB_Settings' ) || ! BoldThemes_BB_Settings::$custom_content_elements ) {
	...
	require_once 'admin-notice.php'; // ### Instagram ###
}
