<?php
// new galleries template for custom fields in different categories
$_html = '';
if ( isset($fields) ) {
        $showinfo       = isset( $showinfo ) ? $showinfo : 0;
        $gallery_type   = isset( $gallery_type ) ? $gallery_type : 'grid';
        ...
        foreach ( $fields as $field){          
                if ( isset($field['name']) && isset($field['value']) ) {
                        ...
                        if ( count($field['value']) == 1 ){
                             $galleries[ $key ] = $field['value'][0];
                        } else {
                             $galleries[ $key ] = $field['value'];
                        }
                }
       } 
       if ( empty($galleries) ){
