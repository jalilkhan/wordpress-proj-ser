...
if ( isset($field) ) {	
    if ( isset($field['name']) && isset($field['value']) ) {
        ...
        $rid            = !empty($opentable[0]) ? $opentable[0] : 0;
        $domain_ext     = !empty($opentable[1]) ? $opentable[1] : 'com';
        $show_labels    = true;
        $show_icons     = false;
	...
	<div class="<?php echo $field['slug'] ?> <?php echo $field['group'];?>">
		<?php if ( $title == '' ) : ?>
		<h4><span><?php _e( 'Book with Opentable', 'bt_plugin' ) ?></span></h4>
		<?php endif; ?>

		<div class="btOpenTableReservation">			
			<?php if ( !empty($rid) && intval($rid) ) : ?>
				...
				<div class="btOpenTableReservationRow">
					...
					<div class="btOpenTableReservationColumn btOpenTableReservationColumnSubmit">
						<button type="submit" class="otreservations-submit"><?php echo (__( 'Find a table', 'bt_plugin' )) ?></button>
					</div>
				</div>
