...
if ( ! function_exists( 'boldthemes_get_new_media_html' ) ) {
	function boldthemes_get_new_media_html( $arg = array() ) { 
		...
		if ( $video != '' ) {
			...
			$html = '<div class="btMediaBox btDarkSkin btLink"' . $bt_link_bg_image . '><blockquote><p><a href="' . esc_url( $link_url ) . '">' . $link_title . '</a></p><cite><a href="' . esc_url( $link_url ) . '">' . $link_url . '</a></cite></blockquote></div>';
			...
			$html = '<div class="btMediaBox btQuote btDarkSkin"' . $bt_quote_bg_image . '><blockquote><p>' . $html . '</p><cite>' . $quote_author . '</cite></blockquote></div>';
			...
			$html = '<img src="' . esc_url( $featured_image ) . '" alt="' . esc_url( $featured_image ) . '"/>';
...
if ( ! function_exists( 'boldthemes_pagination' ) ) {
	function boldthemes_pagination() {
		...
		$prev = get_previous_posts_link( esc_html__( 'Newer Posts', 'bello' ) );
		$next = get_next_posts_link( esc_html__( 'Older Posts', 'bello' ) );
...
if ( ! function_exists( 'boldthemes_theme_comment' ) ) {
	function boldthemes_theme_comment( $comment, $args, $depth ) {
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
			<p><?php echo esc_html__( 'Pingback:', 'bello' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'bello' ), '<span class="edit-link">', '</span>' ); ?></p>
		<?php
				break;
			default :
				...
				<div class="commentTxt">
					<div class="vcard divider">
						<?php
							...
							echo '<p class="posted">' . sprintf( esc_html__( '%1$s at %2$s', 'bello' ), get_comment_date(), get_comment_time() ) . '</p>';
							if ( $rating > 0 && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) { ?>
								<div itemscope itemtype="http://schema.org/Rating" class="star-rating" title="<?php echo sprintf( esc_html__( 'Rated %d out of 5', 'bello' ), $rating ) ?>">
									<span style="width:<?php echo esc_html( ( $rating / 5 ) * 100 ); ?>%"><strong itemprop="ratingValue"><?php echo esc_html( $rating ); ?></strong> <?php echo esc_html__( 'out of 5', 'bello' ); ?></span>
								</div>
							<?php }
						?>
					</div>
					...
					<?php if ( '0' == $comment->comment_approved ) : ?>
						<p class="comment-awaiting-moderation"><?php echo esc_html__( 'Your comment is awaiting moderation.', 'bello' ); ?></p>
					<?php endif; ?>
					...
					<div class="comment">
						...
						if ( comments_open() ) {
							echo '<p class="reply">';
								comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'bello' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) );
							echo '</p>';
						}
						edit_comment_link( esc_html__( 'Edit', 'bello' ), '<p class="edit-link">', '</p>' ); ?>
					</div>
				</div>
...
if ( ! function_exists( 'boldthemes_set_override' ) ) {
	function boldthemes_set_override() {
		...
		if ( ! is_404() ) {
			...
			if ( ( is_singular( 'post' ) || is_singular( 'attachment' ) ) && isset( $tmp_boldthemes_page_options[ BoldThemesFramework::$pfx . '_blog_settings_page_slug' ] ) && $tmp_boldthemes_page_options[ BoldThemesFramework::$pfx . '_blog_settings_page_slug'] != '' ) { 
			...
			} else if ( ( is_singular( 'post' ) || is_singular( 'attachment' ) ) && isset( $boldthemes_options['blog_settings_page_slug'] ) && $boldthemes_options['blog_settings_page_slug'] != '' ) {
...
if ( ! function_exists( 'boldthemes_get_body_class' ) ) {
	function boldthemes_get_body_class( $extra_class ) {
		...
		if ( ! ( ( BoldThemesFramework::$sidebar == 'left' || BoldThemesFramework::$sidebar == 'right' ) && ! is_404() )
			...
			||
			( ( ! function_exists( 'is_woocommerce' ) || ! is_woocommerce() )  && ! is_active_sidebar( 'primary_widget_area' ) )
			) {
...
if ( ! function_exists( 'boldthemes_set_global_uri' ) ) {
	function boldthemes_set_global_uri() {
		...
		$data .= 'window.boldthemes_text.previous = \'' . esc_html__( 'previous', 'bello' ) . '\';';
		$data .= 'window.boldthemes_text.next = \'' . esc_html__( 'next', 'bello' ) . '\';';
		return $data;
	}
}
...
if ( ! function_exists( 'boldthemes_get_post_author' ) ) {
	function boldthemes_get_post_author( $author_url = false, $prefix = 'by', $show_avatar = false ) {
		if ( $prefix == 'by' ) $prefix = esc_html__( 'by', 'bello' );
		$post = get_post();
		...
		if ( ! is_404() && ! is_front_page() ) {
			...
			if ( ! $simple ) {
				...
				if ( ! is_singular() || is_page() ) {
					$output .= '<span><a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'bello' ) . '</a></span>';
				}
			} else {
				if ( ! is_singular() || is_page() ) {
					$output .= '<a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'bello' ) . '</a>';
				}
			}
			...
			if ( is_home() ) {
				...
				//if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) {
					...
					$subtitle = '<span class = "btProductSKU">' . esc_html__( 'SKU:', 'bello' ) . ' ' . $sku . '</span>'; 
