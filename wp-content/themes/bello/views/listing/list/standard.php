...
$listing_search_distance_radius     = boldthemes_get_option( 'listing_search_distance_radius' ) != '' ? boldthemes_get_option( 'listing_search_distance_radius' ) : '100000';
...
$map_center     = boldthemes_get_listings_google_map_center( BoldThemesFrameworkTemplate::$listings );
...
if ( $listing_api_key != '' ) {
        if ( !wp_script_is( 'gmaps_api_autocomplete_search', 'enqueued' ) ) {
                wp_enqueue_script( 
                        ...
                        'https://maps.googleapis.com/maps/api/js?key=' . $listing_api_key . '&libraries=places&callback=bt_bb_listing_gmap_init&language=' . $listing_map_localization . '#asyncload'
                );
        ...
        wp_enqueue_script( 
                'gmaps_api',
                'https://maps.googleapis.com/maps/api/js?callback=bt_bb_listing_gmap_init&language=' . $listing_map_localization . '#asyncload'
        );
}

wp_register_script( 'bello_listing_custom_js', get_template_directory_uri() . '/views/listing/js/custom.js' );
wp_localize_script( 'bello_listing_custom_js', 'ajax_object_map', array(
	'pin_normal'		=> $listing_pin_normal , 
	'pin_selected'		=> $listing_pin_selected,
...
wp_enqueue_script( 'bello_listing_custom_js' );
...
wp_localize_script( 'bello_listing_search_js', 'ajax_object', array( 
	...
	'ajax_action_map'			=> 'bt_get_listing_results_map_action',
        'ajax_lat'                              => BoldThemesFrameworkTemplate::$bt_bb_listing_field_my_lat, 
	'ajax_lng'                              => BoldThemesFrameworkTemplate::$bt_bb_listing_field_my_lng,
        'ajax_unit'                             => $listing_search_distance_unit,
	'ajax_radius'                           => $listing_search_distance_radius,
						...
						<div class="bt_bb_listing_search_parameters">
							...
							<div class="bt_bb_listing_search_inner">
								<?php get_template_part( 'views/listing/list/search' ); ?>
                                                                <?php
                                                                    wp_register_script( 'boldthemes-script-standard-1', '' );
                                                                    wp_enqueue_script( 'boldthemes-script-standard-1' );
                                                                    wp_add_inline_script( 'boldthemes-script-standard-1', 'var user_location = {};' );
                                                                ?>
